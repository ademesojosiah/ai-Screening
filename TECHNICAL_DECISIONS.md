...
This document records the technical decisions, current decisions, and active plans for HireFlow. It should stay practical: every product problem should map to an implemented or planned technical response.
...
- AI screening should surface:
  ...
  - match percentage
  - short explanation
- Current basic worker output also separates:
  - resume/job analysis score, explanation, and review
...
- Keep staged score sections separate so HR recommendations can combine them later without losing the original explanations.
...
- Never auto-reject from project consistency alone.
...
- Notification service receives `APPLICATION_STAGE_UPDATED` events and broadcasts them to connected clients with Server-Sent Events.
...
- Broadcast application actions/stages only after the Application Service has persisted the change.
